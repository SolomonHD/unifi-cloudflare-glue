...
- **Terraform State Persistence in Integration Tests (fix-terraform-state-persistence):**
  - Fixed critical bug where Terraform state was lost between create and destroy phases
  - State is now exported after successful Cloudflare and UniFi resource creation
  - State is imported before cleanup destruction to ensure proper resource tracking
  - Added graceful fallback to config-based destroy if state operations fail
  - Cleanup summary now indicates state-based vs config-based cleanup method
  - Prevents orphaned Cloudflare tunnels, DNS records, and UniFi DNS records
  - Files modified: `src/main/main.py` (`test_integration` function)
